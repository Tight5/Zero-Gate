...
export const GrantForm: React.FC<GrantFormProps> = ({
  ...
  useEffect(() => {
    if (watchedValues.submissionDeadline && watchedValues.milestones?.length === 0) {
  ...
  const generateAutoMilestones = (submissionDate: Date) => {
    const legacyMilestones = generateCompatibleMilestones(submissionDate);
    const validatedMilestones = validateMilestoneData(legacyMilestones);
    form.setValue('milestones', validatedMilestones);
  };
  ...
  const handleFormSubmit = (data: FormData) => {
    // Transform milestones to include validation schema format
    const processedData = {
      ...data,
      milestones: validateMilestoneData(data.milestones || [])
    };
  ...
  const addTeamMember = () => {
    ...
    form.setValue('teamMembers', [...teamMembers, { name: '', role: '', email: '' }]);
  };

  const removeTeamMember = (index: number) => {
    const teamMembers = form.getValues('teamMembers') || [];
    form.setValue('teamMembers', teamMembers.filter((_, i) => i !== index));
  };

  const addDocument = () => {
    const documents = form.getValues('requiredDocuments') || [];
    form.setValue('requiredDocuments', [...documents, { name: '', type: 'pdf', required: true }]);
  };

  const removeDocument = (index: number) => {
    const documents = form.getValues('requiredDocuments') || [];
    form.setValue('requiredDocuments', documents.filter((_, i) => i !== index));
  };

  const getStepIcon = (step: number) => {
    if (stepValidation[step]) return <CheckCircle className="w-5 h-5 text-green-500" />;
